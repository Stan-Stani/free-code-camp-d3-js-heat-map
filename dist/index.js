...
function buildScales(data) {
    ...
    legend
        ...
        .append('rect')
            ...
            .attr('width',  (d, i) => {
                if (!colorLengths[colorLengths.length - 1].length) {
                    
                     return (LEGEND_LENGTH * 
                    (d - (colorLengths[i-1] ? colorLengths[i-1] : 0)) / colorLengths[colorLengths.length - 1]);
                } else {
                    return (LEGEND_LENGTH * (d - (colorLengths[i-1] ? colorLengths[i-1] : 0)) / colorLengths[colorLengths.length - 1][1]);
                }
            })
